...
**Layout — strict 1:1 mirror.** Every package module has exactly one test file at the mirrored path (`src/moodengine/cluster.py` → `tests/unit/test_cluster.py`). New module ⇒ new mirrored test file in the same change. No test code inside the package, ever. Acted exception: `embeddings/clap.py`, `embeddings/mert.py` and `embeddings/mulan.py` are pure torch wrappers — no unit-test files; they are covered by the opt-in `-m model` integration suite. Do not "fix" this by adding mocked unit tests for them.
...
ships the `pot` distribution, but it imports as `ot`), `leidenalg`, `pacmap`, `shap`, `muq`.
